#!/usr/bin/env python3
"""
Test: Safari browser basic operations
Tests navigation, tabs, windows, and bookmarks

Prerequisites:
1. fsq-mac installed
2. Appium server running
3. macOS Accessibility permission granted
"""

import subprocess
import json
import time
import sys
from typing import Optional, Dict, Any

MAC_CLI = "mac"


def run_mac(*args, check=True) -> Optional[Dict[str, Any]]:
    """Run mac CLI command and return parsed JSON output"""
    cmd = [MAC_CLI] + list(args)
    print(f"    $ {' '.join(cmd)}")
    
    result = subprocess.run(cmd, capture_output=True, text=True)
    
    if result.returncode != 0 and check:
        return None
    
    try:
        return json.loads(result.stdout) if result.stdout.strip() else {}
    except json.JSONDecodeError:
        return {"raw": result.stdout}


def send_hotkey(*keys) -> bool:
    """Send hotkey combination"""
    result = run_mac("input", "hotkey", *keys, check=False)
    time.sleep(0.3)
    return result is not None and result.get("ok", True)


def type_text(text: str) -> bool:
    """Type text"""
    result = run_mac("input", "type", text, check=False)
    time.sleep(0.2)
    return result is not None


def check_environment() -> bool:
    """Check critical environment requirements"""
    print("\n[Setup] Checking environment...")
    doctor = run_mac("doctor", check=False)
    if doctor:
        checks = doctor.get("error", {}).get("details", {}).get("checks", [])
        for check in checks:
            if check.get("name") in ["accessibility_permission", "appium_server"]:
                if check.get("status") != "pass":
                    print(f"    ❌ {check.get('name')} failed")
                    return False
    print("    ✅ Environment OK")
    return True


def setup_session() -> bool:
    """Start session and launch Safari"""
    print("\n[Setup] Starting session...")
    run_mac("session", "start", check=False)
    
    subprocess.run(
        ["osascript", "-e", 'tell application "Safari" to activate'],
        capture_output=True
    )
    time.sleep(1)
    print("    ✅ Safari activated")
    return True


def navigate_to_url(url: str) -> bool:
    """Navigate to a URL"""
    print(f"\n  Navigating to {url}...")
    send_hotkey("command", "l")
    time.sleep(0.2)
    type_text(url)
    send_hotkey("return")
    time.sleep(2)
    return True


# =============================================================================
# Test Cases
# =============================================================================

def test_new_window() -> bool:
    """Test: Create new window"""
    print("\n" + "=" * 50)
    print("Test 1: New Window (Cmd+N)")
    print("=" * 50)
    
    print("\n  Creating new window...")
    send_hotkey("command", "n")
    time.sleep(1)
    
    print("    ✅ New window created")
    return True


def test_new_tab() -> bool:
    """Test: Create new tab"""
    print("\n" + "=" * 50)
    print("Test 2: New Tab (Cmd+T)")
    print("=" * 50)
    
    print("\n  Creating new tab...")
    send_hotkey("command", "t")
    time.sleep(0.5)
    
    print("    ✅ New tab created")
    return True


def test_navigate_url() -> bool:
    """Test: Navigate to URL"""
    print("\n" + "=" * 50)
    print("Test 3: Navigate to URL")
    print("=" * 50)
    
    navigate_to_url("https://example.com")
    
    print("    ✅ Navigation completed")
    return True


def test_new_private_window() -> bool:
    """Test: Create new private window"""
    print("\n" + "=" * 50)
    print("Test 4: New Private Window (Cmd+Shift+N)")
    print("=" * 50)
    
    print("\n  Creating private window...")
    send_hotkey("command", "shift", "n")
    time.sleep(1)
    
    print("    ✅ Private window created")
    return True


def test_add_bookmark() -> bool:
    """Test: Add bookmark"""
    print("\n" + "=" * 50)
    print("Test 5: Add Bookmark (Cmd+D)")
    print("=" * 50)
    
    # Navigate to a page first
    navigate_to_url("https://apple.com")
    
    print("\n  Adding bookmark...")
    send_hotkey("command", "d")
    time.sleep(0.5)
    
    # Cancel to avoid clutter
    send_hotkey("escape")
    time.sleep(0.3)
    
    print("    ✅ Bookmark dialog opened (cancelled)")
    return True


def test_show_bookmarks() -> bool:
    """Test: Show bookmarks sidebar"""
    print("\n" + "=" * 50)
    print("Test 6: Show Bookmarks (Cmd+Ctrl+1)")
    print("=" * 50)
    
    print("\n  Showing bookmarks sidebar...")
    send_hotkey("command", "control", "1")
    time.sleep(0.5)
    
    print("    ✅ Bookmarks sidebar shown")
    
    # Hide sidebar
    send_hotkey("command", "shift", "l")
    time.sleep(0.3)
    
    return True


def test_switch_tabs() -> bool:
    """Test: Switch between tabs"""
    print("\n" + "=" * 50)
    print("Test 7: Switch Tabs (Cmd+Shift+] and [)")
    print("=" * 50)
    
    # Create multiple tabs
    send_hotkey("command", "t")
    time.sleep(0.3)
    navigate_to_url("https://github.com")
    
    print("\n  Switching to previous tab...")
    send_hotkey("command", "shift", "[")
    time.sleep(0.5)
    
    print("    ✅ Switched to previous tab")
    
    print("\n  Switching to next tab...")
    send_hotkey("command", "shift", "]")
    time.sleep(0.5)
    
    print("    ✅ Switched to next tab")
    return True


def test_close_tab() -> bool:
    """Test: Close tab"""
    print("\n" + "=" * 50)
    print("Test 8: Close Tab (Cmd+W)")
    print("=" * 50)
    
    print("\n  Closing tab...")
    send_hotkey("command", "w")
    time.sleep(0.3)
    
    print("    ✅ Tab closed")
    return True


def test_reload_page() -> bool:
    """Test: Reload page"""
    print("\n" + "=" * 50)
    print("Test 9: Reload Page (Cmd+R)")
    print("=" * 50)
    
    print("\n  Reloading page...")
    send_hotkey("command", "r")
    time.sleep(1)
    
    print("    ✅ Page reloaded")
    return True


def test_back_forward() -> bool:
    """Test: Navigate back and forward"""
    print("\n" + "=" * 50)
    print("Test 10: Back/Forward Navigation")
    print("=" * 50)
    
    # Navigate to create history
    navigate_to_url("https://example.com")
    navigate_to_url("https://apple.com")
    
    print("\n  Going back...")
    send_hotkey("command", "[")
    time.sleep(1)
    
    print("    ✅ Navigated back")
    
    print("\n  Going forward...")
    send_hotkey("command", "]")
    time.sleep(1)
    
    print("    ✅ Navigated forward")
    return True


def teardown() -> None:
    """Cleanup after tests"""
    print("\n[Teardown] Cleaning up...")
    
    # Close all windows
    for _ in range(4):
        send_hotkey("command", "w")
        time.sleep(0.2)
    
    run_mac("session", "end", check=False)
    print("    ✅ Cleanup complete")


def main():
    """Main test runner"""
    print("\n" + "=" * 60)
    print("  Safari Test Suite - Basic Operations")
    print("=" * 60)
    
    results = {}
    
    try:
        if not check_environment():
            print("\n❌ Environment check failed")
            sys.exit(1)
        
        setup_session()
        
        # Run tests
        results["new_window"] = test_new_window()
        results["new_tab"] = test_new_tab()
        results["navigate_url"] = test_navigate_url()
        results["new_private_window"] = test_new_private_window()
        results["add_bookmark"] = test_add_bookmark()
        results["show_bookmarks"] = test_show_bookmarks()
        results["switch_tabs"] = test_switch_tabs()
        results["close_tab"] = test_close_tab()
        results["reload_page"] = test_reload_page()
        results["back_forward"] = test_back_forward()
        
    except KeyboardInterrupt:
        print("\n⚠️ Test interrupted")
        sys.exit(130)
    except Exception as e:
        print(f"\n❌ Test failed with error: {e}")
        import traceback
        traceback.print_exc()
        sys.exit(1)
    finally:
        teardown()
    
    # Summary
    print("\n" + "=" * 60)
    print("  Test Summary")
    print("=" * 60)
    
    passed = sum(1 for v in results.values() if v)
    for name, result in results.items():
        status = "✅" if result else "❌"
        print(f"  {status} {name}")
    
    print(f"\n  Total: {passed}/{len(results)} passed")
    
    sys.exit(0 if passed == len(results) else 1)


if __name__ == "__main__":
    main()
